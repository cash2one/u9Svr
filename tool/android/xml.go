...
func LoadXmlFile(file string) (result *Element) {
	byteArray, err := ioutil.ReadFile(file)
	if err != nil {
		panic(err)
	}

	result, err = LoadByXml(string(byteArray))
	if err != nil {
		panic(err)
	}
	return
...
func LoadByStream(r io.Reader) (current *Element, err error) {
	xmlSpaceName := map[string]string{
		...
		case xml.StartElement:
			...
			for _, a := range token.Attr {
				...
				if a.Name.Space == "xmlns" {
					xmlSpaceName[a.Value] = a.Name.Local
				}
				if a.Name.Space != "" {
					if spaceName, ok := xmlSpaceName[a.Name.Space]; ok {
						ar.name = spaceName + ":" + a.Name.Local
					} else {
...
func (t *Element) _string() string {
	...
	sattr := ""
	if len(t.Attrs) > 0 {
		isReturn := len(t.Attrs) > 2
		for index, att := range t.Attrs {
			if index != len(t.Attrs)-1 && isReturn {
...
func (t *Element) GetNodeByPathAndAttr(path, attrName, attrValue string) *Element {
	els := t.GetNodesByPath(path)
	if len(els) > 0 {
		for _, el := range els {
...
func (t *Element) ClearAttr() {
	...
	t.Attrs = make([]*Attr, 0)
	t.attrmap = make(map[string]string, 0)
}
...
func (t *Element) RemoveNode(nodeName, attrName, attrValue string) bool {
	if t._root().isSync {
		t._root().lc.Lock()
		defer t._root().lc.Unlock()
	} else {
	...
	childs, ok := t.elementmap[nodeName]
	if ok {
		newCs := make([]E, 0)
		for _, child := range childs {
			...
			if !find {
				newCs = append(newCs, el)
			}
		}
		t.elementmap[nodeName] = newCs

		newCs = make([]E, 0)
		for _, child := range t.childs {
			el := child.(*Element)
			if el.name != nodeName {
				newCs = append(newCs, child)
			} else {
				find := false
				for _, attr := range el.Attrs {
					if attr.name == attrName && attr.Value == attrValue {
...
func (t *Element) RemoveNodes(name string) bool {
	if t._root().isSync {
		t._root().lc.Lock()
		defer t._root().lc.Unlock()
	} else {
...
func (t *Element) ToXML() string {
	...
	return xml.Header + t._root()._string()
}
...
func (t *Element) MergeByName(mergeEl *Element, elName string) {
Find:
	for _, mergeChild := range mergeEl.AllNodes() {
		if elName == mergeChild.Name() {
			for _, child := range t.AllNodes() {
				if elName == child.Name() {
					t.MergeAttrs(mergeChild)
					break Find
				}
			}
			t.AddNode(mergeChild)
			break Find
		}
	}
}

func (t *Element) MergeByAttr(mergeEl *Element, attrName string) {
	for _, mergeChild := range mergeEl.AllNodes() {
		find := false
		for _, el := range t.AllNodes() {
			attrValue, mbOk := el.AttrValue(attrName)
			mergeAttrValue, clOk := mergeChild.AttrValue(attrName)
			if el.Name() == mergeChild.Name() && mbOk && clOk && mergeAttrValue == attrValue {
				find = true
				break
			}
		}
...
func (t *Element) MergeByNameAndAttr(mergeEl *Element, elName, attrName string) {
	for _, mergeChild := range mergeEl.AllNodes() {
		if elName == mergeChild.Name() {
			find := false
			for _, el := range t.AllNodes() {
				attrValue, mbOk := el.AttrValue(attrName)
				mergeAttrValue, clOk := mergeChild.AttrValue(attrName)
				if mergeChild.Name() == el.Name() && mbOk && clOk && mergeAttrValue == attrValue {
					find = true
					break
				}
			}
...
func (t *Element) MergeAttrs(mergeEl *Element) {
	for mAttrName, mAttrValue := range mergeEl.attrmap {
		_, ok := t.AttrValue(mAttrName)
		if !ok {
			t.AddAttr(mAttrName, mAttrValue)
		}
	}
}
